...
function heartbeat(){
	global $response, $userid, $chatrooms_language, $chatroomTimeout, $lastMessages, $cookiePrefix, $allowAvatar, $guestsMode, $crguestsMode, $guestnamePrefix, $chromeReorderFix, $showChatroomUsers, $showGroupsOnlineUsers, $force, $groupid, $password, $groupname, $encodedgroupname, $initialize, $callbackfn;
	...
	if(!empty($groupid) && $userid>0){
		...
		if($group = sql_fetch_assoc($query)){
			...
			if($owner==0){
				...
				$query = sql_query('deleteGroup',array('id'=>$groupid));
	...
	if(!empty($groupid) && $userid>0){
		...
		if($group = sql_fetch_assoc($query)){
			...
			if($owner==0){
				...
				if(sql_num_rows($query) == 0){
					$query = sql_query('renameGroup', array('name'=>$groupname,'id'=>$groupid));
					$affectedrow =  sql_affected_rows($query);
					if($affectedrow>0){
						$response['success'] = true;
