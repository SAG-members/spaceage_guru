...
$action = base_url('admin/sensations/add');
$status = 'disabled';
...
if(isset($page))
{
	...
	$action = base_url('admin/sensations/edit/'.$page->{Page::_ID});
			...
			<div class="x_content">
				...
				<form id="demo-form2" action="<?php echo $action;?>"  method="post" class="form-horizontal form-label-left">
					...
					<div class="form-group mar-t-20">
						<input type="text" class="form-control" name="title" placeholder="Service Name" value="<?php echo $page_title;?>">
					</div>
					<div class="form-group mar-t-20">
...
$(function(){
	...
	/* On Page Load Check If Id Exists*/
	
	<?php if(isset($page)) :?>

		payload = {'pageId':<?php echo $page->{Page::_ID};?>}
		request = {'acid':10011,'payload':JSON.stringify(payload)};
		
		var url = BASE_URL+'admin/ajax';
		var data = {request:request};
		
		startAjaxRequest(url, data).done(function(response)
		{
			$.each(response, function(k,v){
				if(v != "")
				{
					$(JSON.parse(v)).each(function(i,j){
						$('input[type="radio"][name="'+j.k+'"][value="'+j.v+'"]').prop('checked', true);
						$('input[type="text"][name="'+j.k+'"]').val(j.v);
					});
				}
			});
		});
